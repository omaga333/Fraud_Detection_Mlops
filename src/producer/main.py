...
# Load environment variables (Adjusted path for Docker if needed)
...
class TransactionProducer:
    def __init__(self):
        # Default to kafka:29092 for Docker local network
        ...
        # Explicitly check security protocol instead of just username/password presence
        ...
        try:
            ...
            logger.info("Confluent Kafka Producer initialized successfully with %s", self.security_protocol)
    ...
    def run_continuous_production(self, interval: float = 0.05): # Added small delay to prevent CPU overload
    ...
    def shutdown(self, signum=None, frame=None):
        ...
        if self.running:
            ...
            if hasattr(self, 'producer') and self.producer:
                self.producer.flush(timeout=30)  # <-- Flush only, close() is removed for confluent_kafka
